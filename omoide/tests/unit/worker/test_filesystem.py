...
def test_filesystem_safely_save(valid_worker_config, worker_dt):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with mock.patch('omoide.omoide_worker.filesystem.pu') as fake_pu:
            fake_now = fake_pu.now
...
def test_filesystem_make_new_filename(filename, reference, worker_dt, valid_worker_config):
