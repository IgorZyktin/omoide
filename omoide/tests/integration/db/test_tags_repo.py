...
def _user_rows(engine, user_id: int) -> dict[str, int]:
    ...
    with engine.connect() as conn:
        rows = conn.execute(
            sa.select(db_models.KnownTags.tag, db_models.KnownTags.counter).where(
                db_models.KnownTags.user_id == user_id
            )
        ).fetchall()
...
class TestIncrementKnownTagsUser:
    ...
    async def test_clamps_negative_counter_to_zero_then_adds_one(
        ...
        """Test that we do not go into negative counters when adding rows.
    ...
    async def test_handles_mix_of_known_and_unknown_tags(
        ...
        async with async_database.transaction() as conn:
            await tags_repo.increment_known_tags_user(conn, user, {'old', 'fresh-1', 'fresh-2'})
...
class TestIncrementKnownTagsAnon:
    ...
    async def test_creates_row_for_unknown_tag(self, async_database, tags_repo, engine):
        async with async_database.transaction() as conn:
    ...
    async def test_increments_and_creates_in_one_call(
        ...
        async with async_database.transaction() as conn:
            await tags_repo.increment_known_tags_anon(conn, {'known', 'unknown'})
...
class TestDecrementKnownTagsUser:
    ...
    async def test_does_not_create_row_for_unknown_tag(
        ...
        """Test that we do no create rows for unknown tags.
        ...
        async with async_database.transaction() as conn:
            await tags_repo.decrement_known_tags_user(conn, user, {'never_seen'})
...
class TestDecrementKnownTagsAnon:
    ...
    async def test_does_not_create_row_for_unknown_tag(self, async_database, tags_repo, engine):
