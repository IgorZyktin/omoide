...
class UploadCommand(Command):
    ...
    async def execute(self) -> int:
        ...
        # problems trying to download this file later, so we're altering its
        # timestamps.
        stat = await aiofiles.os.stat(content_path)
        oldest_timestamp = min(stat.st_ctime, stat.st_atime, stat.st_mtime)
        if oldest_timestamp < const.OLDEST_ALLOWED_TIMESTAMP:
            try:
                ...
                LOG.exception(
                    'Failed to update timestamps on a file, '
                    'oldest timestamp is {}: {}',
                    oldest_timestamp,
                    content_path,
                )
        ...
        for label, existed, path in [
            ...
            if existed:
                LOG.warning(
                    '[{}] Overwrote {} file: {}', self.dto.id, label, path
                )
            else:
